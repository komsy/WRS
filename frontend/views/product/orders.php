...
$orders= Orderitems::find()->joinWith('order')->where(['userId'=>Yii::$app->user->id])->joinWith('cans')->joinWith('product')->all();
        ...
        <?php foreach ($orders as $listing ) {?>            
         ...
         <?php } ?>
                        ...
                        <div class="row shadow card-footer">
                            ...
                            <div class="col-md-6">
                              <h1>Total Amount: Ksh <?=$totalamount ?>  </h1>         
                            </div>
                            <div class="col-md-4">
                                <!--  <a href="#" baseUrl="<?= Yii::$app->request->baseUrl?>" productid="<?= $listing->product->productId?>" total="<?=$totalamount?>" userid="<?= Yii::$app->user->id?>" class="btn btn-lg btn-success pull-right text-uppercase addorder"> Checkout </a> -->
